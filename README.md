...
- 1x Custom PCB for Watch Inputs
  ...
  Altiums Files located in the repository under Watch_Input_PCB, *Errors/Warnings related to missing schematic libraries may show up, associated altium libraries are not included,
  and should be downloaded from the purchase/information sources above.
  Altium Designer version 25.8.1 utilized, due to an altium bug, when reproducing the gerberfiles, in the project output folder delete all mechanical layers (.GM#) and keepout layer (.GKO) (messes up the board size).
  
  Information/Reference: Gerber Files for Manufacturing located at Watch_Input_PCB/Project Outputs for Watch_Input_PCB.zip
